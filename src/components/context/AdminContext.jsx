...
const AdminContextProvider = (props) => {
    ...
    try {
        const response = await fetch(`${backednUrl}/api/client-products?filter=false`);
    ...
    const fetchUsers = async () => {
        try {
            const response = await axios.get(`${backednUrl}/api/auth/users`, { headers: { aToken } });
    ...
    const updateOrderStatus = async (orderId, newStatus) => {
        try {
            ...
            if (response.data.success) {
                ...
                toast.error("Failed to update order status (Backend did not return success)");
            }
    ...
    const listQuotes = async () => {
        ...
        try {
            const { data } = await axios.get(`${backednUrl}/api/checkout/list-quote`, { headers: { aToken } })
            if (data.success) {
